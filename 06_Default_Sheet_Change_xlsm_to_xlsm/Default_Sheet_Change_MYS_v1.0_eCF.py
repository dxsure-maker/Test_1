...
class App:
    def __init__(self, root):
        self.root = root
        self.root.title("eCF Default Sheet Change MYS v1.0")
    ...
    def _build_ui(self):
        ...
        # Main file selector
        ttk.Label(frm, text="기준 파일 선택:").grid(row=0, column=0, sticky="w", **pad)
        ttk.Entry(frm, textvariable=self.main_file_var, width=70).grid(row=0, column=1, sticky="we", **pad)
        ttk.Button(frm, text="파일선택", command=self.pick_main_file).grid(row=0, column=2, **pad)

        # Sheet names
        ttk.Label(frm, text="복제 대상 시트 이름 (복수 입력 시 ,로 구분):").grid(row=1, column=0, sticky="w", **pad)
        ttk.Entry(frm, textvariable=self.sheet_names_var, width=70).grid(row=1, column=1, sticky="we", **pad)

        # Exclude sheets
        ttk.Label(frm, text="제거할 시트 이름 (복수 입력 시 ,로 구분):").grid(row=2, column=0, sticky="w", **pad)
        ttk.Entry(frm, textvariable=self.exclude_sheet_names_var, width=70).grid(row=2, column=1, sticky="we", **pad)

        # Target folder
        ttk.Label(frm, text="복제 대상 폴더 위치:").grid(row=3, column=0, sticky="w", **pad)
        ttk.Entry(frm, textvariable=self.folder_var, width=70).grid(row=3, column=1, sticky="we", **pad)
        ttk.Button(frm, text="폴더선택", command=self.pick_folder).grid(row=3, column=2, **pad)

        # Output subfolder name
        ttk.Label(frm, text="결과 파일 폴더명 (위치는 대상 폴더 하위):").grid(row=4, column=0, sticky="w", **pad)
        ttk.Entry(frm, textvariable=self.output_subfolder_var, width=70).grid(row=4, column=1, sticky="we", **pad)

        # File list
        ttk.Label(frm, text="복제 대상 폴더의 파일 리스트:").grid(row=5, column=0, sticky="w", **pad)
        self.file_list = tk.Listbox(frm, height=6)
        self.file_list.grid(row=5, column=1, sticky="we", **pad)
        frm.grid_columnconfigure(1, weight=1)

        self.refresh_btn = ttk.Button(frm, text="리스트 갱신", command=self.refresh_file_list)
        self.refresh_btn.grid(row=5, column=2, sticky="n", **pad)

        # Progress bar
        ttk.Label(frm, text="진행률:").grid(row=6, column=0, sticky="w", **pad)
        prog_frame = ttk.Frame(frm)
        prog_frame.grid(row=6, column=1, sticky="we", **pad)
        ...
        # ETA row
        ttk.Label(frm, text="남은 시간:").grid(row=7, column=0, sticky="w", **pad)
        ttk.Label(frm, textvariable=self.eta_text).grid(row=7, column=1, sticky="w", **pad)

        # Cancel / Run buttons (stacked)
        self.cancel_btn = ttk.Button(frm, text="중단", command=self.cancel_run)
        self.cancel_btn.grid(row=6, column=2, sticky="new", padx=8, pady=(4, 2))
        self.run_btn = ttk.Button(frm, text="수행", command=self.run)
        self.run_btn.grid(row=7, column=2, sticky="new", padx=8, pady=(2, 6), ipady=6)

        # Log area
        ttk.Label(frm, text="수행 로그:").grid(row=8, column=0, sticky="nw", **pad)
        self.log_box = ScrolledText(frm, height=12, state="disabled")
        self.log_box.grid(row=8, column=1, columnspan=2, sticky="nsew", **pad)
    ...
    def run(self):
        ...
        thread = threading.Thread(
            ...
            args=(main_file, sheet_names, exclude_sheet_names, folder, output_subfolder),
            daemon=True,
        )
        thread.start()
    ...
    def _run_in_thread(self, main_file, sheet_names, exclude_sheet_names, folder, output_subfolder):
