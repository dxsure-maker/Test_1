...
class App:
    ...
    def _build_ui(self):
        ...
        # Exclude sheets
        ttk.Label(frm, text="Exclude sheet name(s) (comma separated):").grid(row=2, column=0, sticky="w", **pad)
        ttk.Entry(frm, textvariable=self.exclude_sheet_names_var, width=70).grid(row=2, column=1, sticky="we", **pad)

        # Target folder
        ttk.Label(frm, text="Target folder (대상 폴더):").grid(row=3, column=0, sticky="w", **pad)
        ttk.Entry(frm, textvariable=self.folder_var, width=70).grid(row=3, column=1, sticky="we", **pad)
        ttk.Button(frm, text="Browse", command=self.pick_folder).grid(row=3, column=2, **pad)

        # Output subfolder name
        ttk.Label(frm, text="Output subfolder (대상 폴더 하위):").grid(row=4, column=0, sticky="w", **pad)
        ttk.Entry(frm, textvariable=self.output_subfolder_var, width=70).grid(row=4, column=1, sticky="we", **pad)

        # File list
        ttk.Label(frm, text="Excel files found in folder:").grid(row=5, column=0, sticky="w", **pad)
        ...
        # ETA row
        ttk.Label(frm, text="ETA:").grid(row=7, column=0, sticky="w", **pad)
