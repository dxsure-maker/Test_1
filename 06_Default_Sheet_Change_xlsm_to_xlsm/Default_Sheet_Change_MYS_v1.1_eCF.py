...
class ExcelCopier:
    ...
    def _build_ecf_start_marker(self, sheet):
        try:
            a2_value = sheet.Range("A2").Value
        except Exception:
            a2_value = None
        normalized = self._normalize_cell_value(a2_value)
        if not normalized:
            return None
        return f"{str(a2_value).strip()}_001"

    def _copy_test_case_partial(self, excel, source_sheet, target_sheet, cancel_cb=None):
        if cancel_cb and cancel_cb():
            return False

        start_marker = self._build_ecf_start_marker(source_sheet)
        end_marker = "END"
        ...
        if not start_marker:
            self.logger("  - PreCondition: source A2 is empty; full copy used.")
            self._copy_sheet_data(excel, source_sheet, target_sheet)
            return True

        source_start_row = self._find_row_in_column(
            source_sheet, 1, start_marker, end_row_hint=source_end_row_bound
        )
        if source_start_row is None:
            self.logger(f"  - PreCondition: '{start_marker}' not found in source column A; full copy used.")
            self._copy_sheet_data(excel, source_sheet, target_sheet)
            return True
        ...
        target_start_marker = self._build_ecf_start_marker(target_sheet)
        if not target_start_marker:
            self.logger("  - PreCondition: target A2 is empty; full copy used.")
            self._copy_sheet_data(excel, source_sheet, target_sheet)
            return True

        target_start_row = self._find_row_in_column(
            target_sheet, 1, target_start_marker, end_row_hint=target_last_row
        )
        if target_start_row is None:
            self.logger(f"  - PreCondition: '{target_start_marker}' not found in target column A; full copy used.")
            self._copy_sheet_data(excel, source_sheet, target_sheet)
            return True
        ...
        if cell_count > max_cells:
            self._copy_range_in_chunks_offset(
                excel,
                source_sheet,
                target_sheet,
                source_start_row,
        ...
        for offset in range(row_count):
            try:
                target_sheet.Rows(target_start_row + offset).RowHeight = \
                    source_sheet.Rows(source_start_row + offset).RowHeight
            except Exception:
                pass
    ...
    def _copy_range_in_chunks_offset(
        self,
        excel,
        source_sheet,
        target_sheet,
...
THEME = {
    "title": "eCF Default Sheet Change MYS v1.1",
    "bg": "#0F172A",
    "panel": "#111827",
    "surface": "#1F2937",
    "surface_alt": "#0B1220",
...
class App:
    def __init__(self, root):
        ...
        self.sheet_names_var = tk.StringVar(value="Test Case")
        self.exclude_sheet_names_var = tk.StringVar(value="")
        self.precondition_var = tk.BooleanVar(value=False)
    ...
    def _build_ui(self):
        ...
        ttk.Label(header, text=THEME["title"], style="HeaderTitle.TLabel").grid(row=0, column=0, sticky="w")
        ...
        self._build_labeled_entry(
            ...
            hint="복제할 시트 이름을 입력합니다. 구분자는 , 를 사용합니다.",
        )
        row += 3
        ...
        ttk.Checkbutton(
            precondition_box,
            text="체크 시 Test Case 시트에서 A2값_001 ~ END 구간만 반영",
        ...
        self._build_labeled_entry(
            ...
            hint="출력 파일에서 제거할 시트 이름을 입력합니다. 구분자는 , 를 사용합니다.",
        )
        row += 3
        self._build_labeled_entry(
            left,
        ...
        self._build_labeled_entry(
            ...
            hint="대상 폴더 하위에 생성할 결과 폴더 이름을 입력합니다. 경로 구분자는 사용할 수 없습니다.",
        )
    ...
    def pick_folder(self):
        path = filedialog.askdirectory(title="복제 대상 폴더 선택")
        if path:
    ...
    def run(self):
        ...
        exclude_sheet_names = [s.strip() for s in self.exclude_sheet_names_var.get().split(",") if s.strip()]
        ...
        for name in sheet_names:
            key = normalize(name)
            if key and key in exclude_keys and key not in seen_overlap:
                overlap.append(name)
                seen_overlap.add(key)

        if not main_file.is_file():
            messagebox.showerror("오류", "올바른 기준 Excel 파일을 선택해 주세요.")
            return
        if not folder.is_dir():
            messagebox.showerror("오류", "올바른 복제 대상 폴더를 선택해 주세요.")
            return
        if not sheet_names:
            messagebox.showerror("오류", "복제할 시트 이름을 하나 이상 입력해 주세요.")
            return
        if any(sep in output_subfolder for sep in ("\\", "/")):
            messagebox.showerror("오류", "출력 폴더명에는 경로 구분자를 넣을 수 없습니다.")
            return
        if overlap:
    ...
    def cancel_run(self):
        if not self.running:
            return
        if messagebox.askyesno("중단", "현재 작업을 중단하시겠습니까?"):
            self.cancel_requested = True
            self._log("중단 요청이 접수되었습니다. 현재 파일 작업이 끝나는 즉시 멈춥니다.")
    ...
    def _run_in_thread(self, main_file, sheet_names, exclude_sheet_names, precondition_enabled, folder, output_subfolder):
        ...
        try:
            ...
            if self.cancel_requested:
                ...
                self._show_message_async("중단", "작업이 중단되었습니다.")
            else:
                self._log("작업이 완료되었습니다.")
                self._show_message_async("완료:", "모든 작업이 완료되었습니다.")
            self.root.after(0, self.refresh_file_list)
        except Exception as exc:
            msg = f"오류: {exc}"
            self._log(msg)
            self._log(traceback.format_exc())
            self._show_error_async("오류", msg)
        finally:
    ...
    def _update_progress(self, current: int, total: int):
        def apply():
            ...
            if eta_sec >= 3600:
                self.eta_text.set(f"{eta_sec / 3600:.1f}시간 남음")
            elif eta_sec >= 120:
                self.eta_text.set(f"{eta_sec / 60:.0f}분 남음")
            elif eta_sec >= 60:
                self.eta_text.set(f"{eta_sec / 60:.1f}분 남음")
            else:
                self.eta_text.set(f"{eta_sec:.0f}초 남음")
